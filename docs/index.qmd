...
ggplot(toplo1, aes(x = day, y = percgerm, group = rep, color = rep)) + 
  geom_line() + 
  geom_point() + 
  ...
  labs(
    title = expression(paste(italic('Pyrodinium bahamense'), ' viable cyst germination by temperature treatment')),
    subtitle = '14 day exposure, three replicates per treatment',
    caption = 'Source: FWC FWRI',
    x = 'Days',
    y = '% germination of viable cysts', 
    color = 'Replicate'
  )
```
...
toplo2 <- germdat |> 
  ...
  summarise(
    avev = mean(percgerm, na.rm = T), 
    hiv = t.test(percgerm, na.rm = T)$conf.int[2], 
    lov = t.test(percgerm, na.rm = T)$conf.int[1],
    .by  = c(temp, day)
  )
